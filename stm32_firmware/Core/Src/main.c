...
int main(void)
{
  ...
  while (1)
  {
	  if(uart_event)
	 	  {
	 		  uart_event = 0;
	           Set_Servo_Angle(&htim2, TIM_CHANNEL_1, value);
	 	  }
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Alternate = GPIO_AF1_TIM2;     // AF1 = TIM2
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
    	uart_event = 1;
        HAL_UART_Transmit(&huart2, rx_data, 3, 10);
        value = (rx_data[0] - '0')*100 + (rx_data[1] - '0')*10 + (rx_data[2] - '0');
        HAL_UART_Receive_IT(&huart2, rx_data, 3);
    }
}
/**
